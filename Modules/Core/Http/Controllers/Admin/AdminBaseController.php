...
class AdminBaseController extends Controller
{
    ...
    protected function safe_destroy($repository, $object, $success_message = 'Registro eliminado correctamente.', $destination_link = null)
    {
        $result = $repository->destroy($object);
        if( is_string( $result ) )
            return redirect()->back()->withError($result);
        else
            if( $destination_link )
                return redirect($destination_link)->withSuccess($success_message);
            else
